...
!developmentChains.includes(network.name)
    ...
    : describe("PostChain Unit Tests", () => {
          ...
          describe("PostChainNft Unit Tests", () => {
              ...
              it("Returns a boolean if postId has been minted", async () => {
                  await network.provider.send("evm_increaseTime", [deadline])
                  await network.provider.send("evm_mine")
                  const isMintedFalse = await postChainNft.getMinted(1)
                  assert.equal(isMintedFalse, false)
                  await postChainNft.mintNft(1)
                  const isMintedTrue = await postChainNft.getMinted(1)
                  assert.equal(isMintedTrue, true)
              })
          })
      })
